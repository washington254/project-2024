...
export default function App() {
  ...
  useEffect(() => {
    if (scrollData) {
      x = mapNumRange(scrollOffset, 0, 1 / (scrollData.pages - 1), 0, scrollData.el.clientHeight);
  ...
  return (
    <main className={`${styles.main} ${device}`}>
      <div className={styles.skillsPage} ref={skillsPageBackgroundRef}>
        <video className="back-video" autoPlay muted loop style={{transform:`translateX(${backVideoX}px)`}}>
          <source src="/stock-market-2023-11-27-05-36-31-utc_compressed.mp4" type="video/mp4"/>
        </video>
      ...
      <Canvas>
        ...
        <ScrollControls pages={4} damping={0.1} maxSpeed={0.5}  >
          <ScrollManager section={section} onSectionChange={setSection}
            setScrollOffset={setScrollOffset} setScrollData={setScrollData} />
          <Scroll >

            <Experience device={device} section={section} cameraRef = {cameraRef} />
          </Scroll>
          <Scroll html>
